...
class SecurityMonitor {
  ...
  private threatLog: (ThreatEvent & { id: number })[];
  private maxLogSize: number;
  private anomalies: Map<string, TrackedRequest[]>;
  private maxTrackedClients: number;
   ...
   */
  detectAnomalies = (req: Request, res: Response, next: NextFunction): void => {
   ...
   */
  monitorAuthAttempts = (req: Request, res: Response, next: NextFunction): void => {
   ...
   */
  cleanupAnomalies(now: number): void {
   ...
   * @param event - Evento de segurança
   */
  logThreatEvent(event: ThreatEvent): void {
    this.threatLog.push({
      ...event,
   ...
   * @param filters - Filtros (tipo, ip, etc)
   * @returns Eventos filtrados
   */
  getThreatReport(filters: { type?: string; ip?: string; since?: number } = {}): (ThreatEvent & { id: number })[] {
   ...
   */
  getThreatStats(): { totalThreats: number; byType: Record<string, number>; byIP: Record<string, number>; recentThreats: (ThreatEvent & { id: number })[] } {
    const stats = {
