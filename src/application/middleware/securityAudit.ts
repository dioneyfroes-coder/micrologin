...
export class SecurityAuditLogger {
   ...
   */
  logSuspiciousActivity(ip: string, userAgent: string, activity: string, details: AuditEventDetails): void {
   ...
   */
  logRateLimitViolation(ip: string, path: string, userAgent: string | undefined, limit: unknown): void {
   ...
   */
  logSecurityAttack(type: string, ip: string, userAgent: string, payload: string, blocked = true): void {
   ...
   */
  getSecurityStats(): AuditStats & { recentEvents: number; activeThreats: number; riskLevel: string } {
   ...
   */
  calculateRiskLevel(recentEvents: SecurityEvent[]): string {
  ...
  generateSecurityReport(): {
    timestamp: string;
    stats: AuditStats & { recentEvents: number; activeThreats: number; riskLevel: string };
   ...
   */
  getTopAttackTypes(): { type: string; count: number }[] {
    const attacks = this.events.filter(e => e.type === 'security_attack');
    const types: Record<string, number> = {};

    attacks.forEach(attack => {
      const type = attack.details.type as string;
   ...
   */
  getTopAttackIPs(): { ip: string; count: number }[] {
